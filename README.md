...
| 데이터 | [Kaggle - Credit Score Classification](https://www.kaggle.com/datasets/parisrohan/credit-score-classification) 기반 `train2.csv` |
...
```
📦 프로젝트 루트
├── credit_score_dl_competition.ipynb   # 전체 분석 노트북
├── train2.csv                          # 학습 데이터
├── credit_score.txt                    # 변수 설명 파일
...
**① 식별자 제거**
- `ID`, `Customer_ID`, `Name`, `SSN`은 예측에 무의미하고 과적합 위험이 있어 제거
...
**⑤ 데이터 누수 방지**
- Imputer·Scaler는 train에만 `fit`, val에는 `transform`만 적용
